...
import com.codelabs.kepuldriver.databinding.ItemOrderAktifBinding
...
class OrderAktifAdapter (val data : ArrayList<OrderResponse.Data?> = arrayListOf()) : RecyclerView.Adapter<OrderAktifAdapter.ViewHolder>() {
    var onClick : ((OrderResponse.Data?) -> Unit?)? = null
    lateinit var sph : SharedPreference
    inner class ViewHolder(val binding: ItemOrderAktifBinding) : RecyclerView.ViewHolder(binding.root) {
        init {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ItemOrderAktifBinding.inflate(LayoutInflater.from(parent.context), parent, false)
...
//            accOrder(holder)
    ...
    fun setData2(datas: List<OrderResponse.Data?>) {
        data.clear()
        data.addAll(datas)
